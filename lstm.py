...
def LSTM():
    ...
    l_forward_2 = lasagne.layers.LSTMLayer(
        l_in_2, N_HIDDEN, grad_clipping=GRAD_CLIP,
        nonlinearity=lasagne.nonlinearities.tanh)
    ...
    #cost = T.nnet.categorical_crossentropy(T.sum(network_output_1*network_output_2,axis = 1), target_values).mean()
    cosine_sim = T.sum(network_output_1*network_output_2,axis = 1) 
    # Retrieve all parameters from the network
    all_params = lasagne.layers.get_all_params(l_out_1, trainable=True) + lasagne.layers.get_all_params(l_out_2, Trainable=True)
